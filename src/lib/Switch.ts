...
@customElement('light-dark-switch-md')
class MDSwitch extends LitElement {
    ...
    connectedCallback() {
        ...
        /**
         * 当调用者的selected没有传递时，根据document.documentElement的类名是否包含dark来确定selected的初始值
         */
        if (this._ayncToLocal() && this.selected == undefined) {
            this.selected = true
        }

        if (this.sync) {
            this._timer = setInterval(() => {
                if (this._ayncToLocal()) {
                    this.selected = true
                } else {
                    this.selected = false
                }
    ...
    protected _toggle() {
        if (this.selected) {
            this._activationDarkClass()
        } else {
            this._removeDarkClass()
        }
    }
...
@customElement('light-dark-switch-standard')
class Switch extends MDSwitch {
    static styles = css`
